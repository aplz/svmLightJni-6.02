...
JavaParamIDs* GetJParamIDs(JNIEnv * env, jobjectArray* tdata) {
	...
	ids->ID_string_format = (*env)->GetFieldID(env, ids->SVMLightModelCls, "m_format",
			"Ljava/lang/String;");
	...
	ids->ID_string_uParam = (*env)->GetFieldID(env, ids->SVMLightModelCls, "m_uParam",
			"Ljava/lang/String;");
	...
	ids->ID_labeledFeatureVectorArray_docs = (*env)->GetFieldID(env, ids->SVMLightModelCls, "m_docs",
			"[Ljnisvmlight/LabeledFeatureVector;");

	if ((ids->ID_string_format && ids->ID_long_kType && ids->ID_long_dParam
			&& ids->ID_double_sParam && ids->ID_double_rParam
			&& ids->ID_string_uParam && ids->ID_long_highFeatIdx
			&& ids->ID_long_trainDocs && ids->ID_long_numSupVecs
			&& ids->ID_double_threshold
			&& ids->ID_labeledFeatureVectorArray_docs) == 0) {
	...
	ids->ConstructorID_SVMLightModelCls = (*env)->GetMethodID(
					env,
					ids->SVMLightModelCls,
					"<init>",
	...
	// ADDED (apilz)
	ids->ID_int_qid = (*env)->GetFieldID(env, ids->tDataCls, "m_qid", "I");

	// ids->MemVarID_size = (*env)->GetFieldID(env, ids->tDataCls, "m_size", "I");
	if (((ids->ID_double_label) && (ids->ID_intArray_dimensions)
			&& (ids->ID_doubleArray_values)) == 0) {
...
JTrainParams* GetJTrainParamIDs(JNIEnv * env, jobject* tparam) {
	...
	tids->ID_LearnParam_lp = (*env)->GetFieldID(env, tparamCls, "m_lp",
			"Ljnisvmlight/LearnParam;");
	tids->ID_KernelParam_kp = (*env)->GetFieldID(env, tparamCls, "m_kp",
			"Ljnisvmlight/KernelParam;");
	...
	if ((tids->ID_int_verbosity && tids->ID_long_type && tids->ID_double_svm_c
			&& tids->ID_double_eps && tids->ID_double_svm_costratio
			&& tids->ID_double_transduction_posratio
			&& tids->ID_long_biased_hyperplane && tids->ID_long_sharedslack
			&& tids->ID_long_svm_maxqpsize && tids->ID_long_svm_newvarsinqp
			&& tids->ID_long_kernel_cache_size && tids->ID_double_epsilon_crit
			&& tids->ID_double_epsilon_shrink
			&& tids->ID_long_svm_iter_to_shrink && tids->ID_long_maxiter
			&& tids->ID_long_remove_inconsistent
			&& tids->ID_long_skip_final_opt_check && tids->ID_long_compute_loo
			&& tids->ID_double_rho && tids->ID_long_xa_depth
			&& tids->ID_string_predfile && tids->ID_string_alphafile
			&& tids->ID_double_epsilon_const && tids->ID_double_epsilon_a
			&& tids->ID_double_opt_precision && tids->ID_long_svm_c_steps
			&& tids->ID_double_svm_c_factor
			&& tids->ID_double_svm_costratio_unlab
			&& tids->ID_double_svm_unlabbound && tids->ID_double_svm_cost
	...
	if ((tids->ID_long_kernel_type && tids->ID_long_poly_degree && tids->ID_double_rbf_gamma && tids->ID_double_coef_lin && tids->ID_double_coef_const && tids->ID_string_custom) == 0) {
	...
	if (tids->argc > 0) {
		...
		for (j=0; j<tids->argc; j++) {
			...
			(tids->argv)[j] = (char*) my_malloc(sizeof(char)
					* strlen(str)+1);
...
void SVMparmInit(KERNEL_CACHE* kernel_cache, LEARN_PARM* learn_parm,
		KERNEL_PARM* kernel_parm, MODEL* model, JTrainParams* tparm) {
	char type[100] = " ";
	jstring test;
	const char *str;
	...
	learn_parm->svm_costratio = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_svm_costratio);
	/* learn_parm->transduction_posratio=-1.0; */
	learn_parm->transduction_posratio = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_transduction_posratio);
	/* learn_parm->biased_hyperplane=1; */
	learn_parm->biased_hyperplane = (*env)->GetLongField(env, tparm->lp,
			tparm->ID_long_biased_hyperplane);
	...
	learn_parm->kernel_cache_size = (*env)->GetLongField(env, tparm->lp,
			tparm->ID_long_kernel_cache_size);
	/* learn_parm->epsilon_crit=0.001; */
	learn_parm->epsilon_crit
			= (*env)->GetDoubleField(env, tparm->lp,
					tparm->ID_double_epsilon_crit);

	learn_parm->epsilon_shrink = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_epsilon_shrink);
	/* learn_parm->svm_iter_to_shrink=-9999; */
	learn_parm->svm_iter_to_shrink = (*env)->GetLongField(env, tparm->lp,
			tparm->ID_long_svm_iter_to_shrink);
	/* learn_parm->maxiter=100000; */
	learn_parm->maxiter = (*env)->GetLongField(env, tparm->lp, tparm->ID_long_maxiter);
	/* learn_parm->remove_inconsistent=0; */
	learn_parm->remove_inconsistent = (*env)->GetLongField(env, tparm->lp,
			tparm->ID_long_remove_inconsistent);
	/* learn_parm->skip_final_opt_check=0; */
	learn_parm->skip_final_opt_check = (*env)->GetLongField(env, tparm->lp,
			tparm->ID_long_skip_final_opt_check);
	...
	learn_parm->epsilon_const = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_epsilon_const);
	/* learn_parm->epsilon_a=1E-15; */
	learn_parm->epsilon_a = (*env)->GetDoubleField(env, tparm->lp, tparm->ID_double_epsilon_a);

	learn_parm->opt_precision = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_opt_precision);

	learn_parm->svm_c_steps = (*env)->GetLongField(env, tparm->lp, tparm->ID_long_svm_c_steps);

	learn_parm->svm_c_factor
			= (*env)->GetDoubleField(env, tparm->lp,
					tparm->ID_double_svm_c_factor);
	/* learn_parm->svm_costratio_unlab=1.0; */
	learn_parm->svm_costratio_unlab = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_svm_costratio_unlab);
	/* learn_parm->svm_unlabbound=1E-5; */
	learn_parm->svm_unlabbound = (*env)->GetDoubleField(env, tparm->lp,
			tparm->ID_double_svm_unlabbound);
	...
	*(learn_parm->svm_cost) = (double) (*env)->GetDoubleField(env, tparm->lp, tparm->ID_double_svm_cost);
	...
	if (argc>0) {
		...
		for (i=0; (i<argc) && ((argv[i])[0] == '-'); i++) {
			...
			case 'y':
				i++;
				printf("Option \"-y\" is not supported in this Version of the JNI-SVMLight-interface!\n");
				fflush(stdout);
				break;
			default:
		...
		} else if (strcmp(type, " ") != 0 || ((learn_parm->type
				& (CLASSIFICATION | REGRESSION | RANKING | OPTIMIZATION))==0)) {
			printf(
	...
	if ((learn_parm->skip_final_opt_check) && (kernel_parm->kernel_type
			== LINEAR)) {
...
(JNIEnv * env, jobject obj, jobjectArray tdata, jobject tparm)
{
	...
	if (verbosity>10) {
		...
		while(z<(*totdoc)) {
			fprintf(dump,"(%ld) (QID: %lf) (CF: %.16g) (SID: %ld) ",docs[z]->docnum,docs[z]->queryid,docs[z]->costfactor,docs[z]->slackid);
...
jobject buildModelData(JNIEnv *env, jobject obj, MODEL* model, JavaParamIDs* ids) {
	...
	if (NUM_DOCS > 1) {
		...
		doks = (*env)->NewObjectArray(env, (jsize) NUM_DOCS-1, ids->tDataCls,
				NULL);
		if (doks == 0) {
		...
		for (u=1; u<NUM_DOCS; u++) {
			...
			jobject Data =(*env)->NewObject(env, ids->tDataCls,
					ids->ConstructorID_tDataCls);
			...
			(*env)->SetDoubleField(env, Data, ids->ID_double_label,
					label);
			...
			for (j=0; j<NUM_FEAT; j++) {
				...
				(*env)->SetIntArrayRegion(env, dim, (jsize) j,
						(jsize) 1, (jint*) &intar[j]);
				(*env)->SetDoubleArrayRegion(env, val, (jsize) j,
						(jsize) 1, (jdouble*) &doublear[j]);
			}

			// Lege Java-Arrays in Objekt des Typs jnisvmlight/LabeledFeatureVector ab
			(*env)->SetObjectField(env, Data,
					ids->ID_intArray_dimensions, dim);
			(*env)->SetObjectField(env, Data,
					ids->ID_doubleArray_values, val);
	...
	char* dummy = (char*) my_malloc(((int)strlen(VERSION)+strlen(text)+1)
			*sizeof(char));
	sprintf(dummy, "%s%s", text, VERSION);

	// kreiere ein neues Objekt vom Typ SVMLightModel 
	jobject SVMLightModel =(*env)->NewObject(env, ids->SVMLightModelCls,
			ids->ConstructorID_SVMLightModelCls, (*env)->NewStringUTF(env, dummy),
			...
			(jdouble) (model->kernel_parm.coef_const), (*env)->NewStringUTF(env, (model->kernel_parm.custom)),
			(jlong) (model->totwords), (jlong) (model->totdoc),
			(jlong) (model->sv_num), (jdouble) (model->b),
	...
	if ( (model->kernel_parm.kernel_type) == 0) {
		...
		for (j=0; j<model->totwords +1; j++) {
			(*env)->SetDoubleArrayRegion(env, val, (jsize) j, (jsize) 1,
					(jdouble*) &(model->lin_weights)[j]);
		}
		(*env)->SetObjectField(env, SVMLightModel,
				ids->ID_doubleArray_linWeights, val);
	} else {
		double nullpointer = 0;
		(*env)->SetObjectField(env, SVMLightModel,
				ids->ID_doubleArray_linWeights, &nullpointer);
	}
...
void createDOCs(JNIEnv * env, JavaParamIDs *JIDs, jobjectArray* tdata,
		DOC*** docs, double** target, long* totwords, long* totdoc,
		long* ndocuments) {
	jboolean ex = 0;
	*ndocuments = 0;
	*totwords = 0;
	*totdoc = (long) JIDs->tDataSize;
	...
	if (verbosity>10) {
		doTest=1;
		if ((test = fopen("jni-train.dat", "w")) == NULL) {
			perror("Writing to \"jni-train.dat\" doesn't work.\n");
			exit(1);
		}
	}
	...
	for (k=0; k<JIDs->tDataSize; k++) {
		...
		//	exit(1);
		//}
		// Lese factor des k-ten Dokuments ein
		jdouble factor = (*env)->GetDoubleField(env, traindoc, JIDs->ID_double_factor);
		jdouble qid = (*env)->GetIntField(env, traindoc, JIDs->ID_int_qid);
		...
		jintArray dim = (*env)->GetObjectField(env, traindoc,
				JIDs->ID_intArray_dimensions);
		jdoubleArray val = (*env)->GetObjectField(env, traindoc,
				JIDs->ID_doubleArray_values);
		if ( (dim && val)== 0) {
		...
		jinit_traindoc((double)label, docs, target, dimLen, totwords, totdoc,
				ds, vs, ndocuments, words, test, qid);
...
void jinit_traindoc(double doc_label, DOC ***docs, double **label,
		...
		double *vals, long* ndocuments, WORD* words, FILE* test, double doc_qid) {
	char comment[1] = { '\0' };
	...
	if (!jparse_document(words, &queryid, &slackid, &costfactor, &wpos,
			max_words_doc, dims, vals)) {
	...
	while (iw<max_words_doc) {
		if (test != NULL)
			fprintf(test, "%ld:%.32g ", words[iw].wnum, words[iw].weight);
		fflush(stdout);
		iw++;
	}
	if (test != NULL)
...
//			create_svector(words, comment, 1.0));
	(*docs)[dnum] = create_example(dnum, doc_qid, slackid, costfactor,
			create_svector(words, comment, 1.0));
...
int jparse_document(WORD* words, long *queryid, long *slackid,
		double *costfactor, long int *numwords, long int max_words_doc,
		int *dims, double *vals) {
	register long wpos;
	long wnum;
	double weight;
